...
- エラー率とは
    ...
    * 混同行列(Confusion Matrix)から読み取る性能評価
        ...
        * 精度,再現率,F1は，あくまで識別対象(Target) $l$ 毎の検出性能
        * 平均性能
          * $マクロ平均 = \frac{\sum_l \text{評価値}_l}{L}$
            * ふつうの「平均」
          * $マイクロ平均 = \frac{\sum_l \{All_l \cdot 評価値_l\} }{\sum_l All_l}$
            * つまり，ラベル毎のサンプル数を重みとする，「重みつき平均」
            * 特に「再現率のマイクロ平均」を **「正解率」** と云う
