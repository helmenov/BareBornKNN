...
double scale_y)          // scale_y
{
  ...
  for(i=-ys;i<ys;i++){
		for(j=-xs;j<xs;j++){
			...
			if(i>0) m=yy+0.5; else m=yy-0.5;      /* 最近傍法 */
			if(j>0) n=xx+0.5; else n=xx-0.5;
...
double scale_y)            // scale
{
  ...
  for(i=-ys;i<ys;i++){
		for(j=-xs;j<xs;j++){
			...
			if(yy>0.0) m=yy; else m=yy-1;
			if(xx>0.0) n=xx; else n=xx-1;
			
			q=yy-m;
			p=xx-n;

			if((m>=-ys)&&(m<ys)&&(n>=-xs)&&(n<xs)){
		  	d=(1.0-q)*((1.0-p)*i_image[(int)m+ys][(int)n+xs]+p*i_image[(int)m+ys][(int)n+1+xs])
				+q*((1.0-p)*i_image[(int)m+1+ys][(int)n+xs]+p*i_image[(int)m+1+ys][(int)n+1+xs]);
			}
			else
		  	d=0.0;
...
double scale_y)            // scale
{
  ...
  for(i=-ys;i<ys;i++){
		for(j=-xs;j<xs;j++){
			...
			if(yy>0) m=yy; else m=yy-1;
			if(xx>0) n=xx; else n=xx-1;
